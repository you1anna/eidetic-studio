...
| Task | Steps | Source |
|---|---|---|
| **Route the kick (or any instrument) to Assign Out 1** — e.g. after a kit/pattern reset dropped it back to Mix Out | **Once, enable the output as audio:** `[UTILITY]` → hold `[SHIFT]`+`[VALUE]` → **Mix Out** → `[ENTER]` → choose **Mode** (Normal or Boost) → `[ENTER]` → select **Assign Out 1** → `[ENTER]`. **Then assign the instrument:** press the **BD (kick) instrument-select** button → hold `[SHIFT]`+`[KIT]` (Kit Edit screen) → hold `[SHIFT]`+`[VALUE]` → **Kit:Output** → `[ENTER]` → turn `[VALUE]` to **ASSIGN 1** → `[ENTER]`. **`[WRITE]`** to save the kit. ⚠ Once on Assign Out, that instrument no longer appears in the Mix Out. | Roland official + Ref. Manual p.47 (2026-07-19) |
